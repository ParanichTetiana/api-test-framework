...
const patientTemplate = require('../testdata/erdx_74752_tc_26.json');
...
describe('NewRx Submission', () => {
  ...
  it('TC26 — Only VtrcksProviderPin invalid, all other fields valid', async () => {
    try {
      ...
      // REQ-SUCCESS: the request succeeds despite the single invalid VtrcksProviderPin.
      ...
      // VtrcksProviderPin persists blank rather than the sent (invalid) value.
      ...
      const validFieldsByColumn = Object.fromEntries(
        Object.entries(expectedFieldsByColumn).filter(([column]) => column !== 'VTRCKS_PROVIDER_PIN')
      );
