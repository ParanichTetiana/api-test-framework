...
const patientTemplate = require('../testdata/erdx_74752_tc_04.json');
...
const { TEST_TIMEOUT_MS, INVALID_CODED_COLUMNS } = require('../constants');
...
describe('NewRx Submission', () => {
  ...
  it('TC4 — Every coded field invalid/unmappable, everything else valid', async () => {
    try {
      ...
      // Invalid/unmappable coded fields should persist blank rather than the sent (invalid) value.
      expectDbFieldsToBeBlank(row, INVALID_CODED_COLUMNS);
      ...
      const validFieldsByColumn = Object.fromEntries(
        Object.entries(expectedFieldsByColumn).filter(([column]) => !INVALID_CODED_COLUMNS.includes(column))
      );
