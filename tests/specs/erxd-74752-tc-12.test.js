...
const patientTemplate = require('../testdata/erdx_74752_tc_12.json');
...
describe('NewRx Submission', () => {
  ...
  it('TC12 — Only AdministrationDate invalid, all other fields valid', async () => {
    try {
      ...
      // REQ-SUCCESS: the request succeeds despite the single invalid AdministrationDate.
      ...
      // AdministrationDate persists blank rather than the sent (invalid) value.
      expectDbFieldsToBeBlank(row, ['ADMINISTRATION_DATE']);
      ...
      const validFieldsByColumn = Object.fromEntries(
        Object.entries(expectedFieldsByColumn).filter(([column]) => column !== 'ADMINISTRATION_DATE')
      );
