...
const patientTemplate = require('../testdata/erdx_74752_tc_25.json');
...
const VALID_COLUMNS = ['REFUSAL', 'MISSED_APT', 'IS_ADMINISTERED', 'CONSENT_TO_REPORT', 'SERIES_COMPLETE'];
...
describe('NewRx Submission', () => {
  ...
  it('TC25 — Only VaccinationSeriesComplete valid, all other rotatable fields invalid', async () => {
    try {
      ...
      // VaccinationSeriesComplete persists its valid value; boolean flags persist Y regardless
      // of validity elsewhere — proves per-field validation isn't skipped once an earlier field fails.
