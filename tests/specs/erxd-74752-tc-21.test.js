...
const patientTemplate = require('../testdata/erdx_74752_tc_21.json');
...
const VALID_COLUMNS = ['REFUSAL', 'MISSED_APT', 'IS_ADMINISTERED', 'CONSENT_TO_REPORT', 'EXPIRATION_DATE'];
...
describe('NewRx Submission', () => {
  ...
  it('TC21 — Only ExpirationDate valid, all other rotatable fields invalid', async () => {
    try {
      ...
      // ExpirationDate persists its valid value; boolean flags persist Y regardless of validity
      // elsewhere — proves per-field validation isn't skipped once an earlier field fails.
