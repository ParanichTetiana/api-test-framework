...
const patientTemplate = require('../testdata/erdx_74752_tc_30.json');
...
describe('NewRx Submission', () => {
  ...
  it('TC30 — Only SerologyResults invalid, all other fields valid', async () => {
    try {
      ...
      // REQ-SUCCESS: the request succeeds despite the single invalid SerologyResults.
      ...
      // SerologyResults persists blank rather than the sent (invalid) value.
      expectDbFieldsToBeBlank(row, ['SEROLOGY_RESULTS']);
      ...
      const validFieldsByColumn = Object.fromEntries(
        Object.entries(expectedFieldsByColumn).filter(([column]) => column !== 'SEROLOGY_RESULTS')
      );
