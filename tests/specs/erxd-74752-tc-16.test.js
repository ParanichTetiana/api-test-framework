...
const patientTemplate = require('../testdata/erdx_74752_tc_16.json');
...
describe('NewRx Submission', () => {
  ...
  it('TC16 — Only RouteOfAdministration invalid, all other fields valid', async () => {
    try {
      ...
      // REQ-SUCCESS: the request succeeds despite the single invalid RouteOfAdministration.
      ...
      // RouteOfAdministration persists blank rather than the sent (invalid) value.
      expectDbFieldsToBeBlank(row, ['ROUTE_OF_ADMINISTRATION']);
      ...
      const validFieldsByColumn = Object.fromEntries(
        Object.entries(expectedFieldsByColumn).filter(([column]) => column !== 'ROUTE_OF_ADMINISTRATION')
      );
