...
const patientTemplate = require('../testdata/erdx_74752_tc_28.json');
...
describe('NewRx Submission', () => {
  ...
  it('TC28 — Only ComorbidityStatus invalid, all other fields valid', async () => {
    try {
      ...
      // REQ-SUCCESS: the request succeeds despite the single invalid ComorbidityStatus.
      ...
      // ComorbidityStatus persists blank rather than the sent (invalid) value.
      expectDbFieldsToBeBlank(row, ['COMORBIDITY_STATUS']);
      ...
      const validFieldsByColumn = Object.fromEntries(
        Object.entries(expectedFieldsByColumn).filter(([column]) => column !== 'COMORBIDITY_STATUS')
      );
