...
const patientTemplate = require('../testdata/erdx_74752_tc_24.json');
...
describe('NewRx Submission', () => {
  ...
  it('TC24 — Only VaccinationSeriesComplete invalid, all other fields valid', async () => {
    try {
      ...
      // REQ-SUCCESS: the request succeeds despite the single invalid VaccinationSeriesComplete.
      ...
      // VaccinationSeriesComplete persists blank rather than the sent (invalid) value.
      expectDbFieldsToBeBlank(row, ['SERIES_COMPLETE']);
      ...
      const validFieldsByColumn = Object.fromEntries(
        Object.entries(expectedFieldsByColumn).filter(([column]) => column !== 'SERIES_COMPLETE')
      );
