...
const patientTemplate = require('../testdata/erdx_74752_tc_18.json');
...
describe('NewRx Submission', () => {
  ...
  it('TC18 — Only LotNumber invalid, all other fields valid', async () => {
    try {
      ...
      // REQ-SUCCESS: the request succeeds despite the single invalid LotNumber.
      ...
      // LotNumber persists blank rather than the sent (invalid) value.
      expectDbFieldsToBeBlank(row, ['LOT_NUMBER']);
      ...
      const validFieldsByColumn = Object.fromEntries(
        Object.entries(expectedFieldsByColumn).filter(([column]) => column !== 'LOT_NUMBER')
      );
