...
const patientTemplate = require('../testdata/erdx_74752_tc_20.json');
...
describe('NewRx Submission', () => {
  ...
  it('TC20 — Only ExpirationDate invalid, all other fields valid', async () => {
    try {
      ...
      // REQ-SUCCESS: the request succeeds despite the single invalid ExpirationDate.
      ...
      // ExpirationDate persists blank rather than the sent (invalid) value.
      expectDbFieldsToBeBlank(row, ['EXPIRATION_DATE']);
      ...
      const validFieldsByColumn = Object.fromEntries(
        Object.entries(expectedFieldsByColumn).filter(([column]) => column !== 'EXPIRATION_DATE')
      );
